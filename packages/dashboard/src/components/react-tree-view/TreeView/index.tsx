...
const useTree = ({
  ...
  const treeParentNode = getTreeRoot(data);
  const [state, dispatch] = useReducer<
    (state: ITreeViewState, action: TreeViewAction) => ITreeViewState
  >(treeReducer, {
    selectedIds: new Set<NodeId>(controlledSelectedIds || defaultSelectedIds),
    controlledIds: new Set<NodeId>(controlledSelectedIds),
    // ! added by ming to fix type error
  ...
  function TreeView(
    ...
    return (
      <ul
        ...
        {...other}
      >
        {getTreeRoot(data).children.map((x, index) => (
          <Node
            key={`${x}-${typeof x}`}
            data={data}
            element={getTreeNode(data, x)}
            setsize={getTreeRoot(data).children.length}
            posinset={index + 1}
            level={1}
            {...state}
            state={state}
