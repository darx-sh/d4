...
export const getLastAccessible = (
  ...
  const isRoot = getTreeRoot(data).id === id;
  if (isRoot) {
    node = getTreeNode(
      data,
      // ! added by ming to fix type error
...
export const getPreviousAccessible = (
  ...
  if (id === getTreeRoot(data).children[0]) {
    return null;
  }
...
export const getTreeRoot = (data: INode[]): INode => {
  const parentNode: INode | undefined = data.find(
    (node) => node.parent === null
  );

  if (!parentNode) {
    throw Error("TreeView data must contain root node.");
  }
